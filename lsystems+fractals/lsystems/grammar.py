...
the input dict, the repeat function is used to repeatedly
...
Stochastic = 
{"A" : {"BCD": 5, "C+C+C":10, "ACA": 40}, "B" : {"DE-F": 5, "CCC":10}}

The Stochastic rule may contain non stochastic elements, in the above 
example there are two stochastic, elements, with keys "A" and "B". The 
stochastic rule is detected by checking the 'value' type is dict. The dict
...
def __weightedRule(rules):
    """
    A private method used to choose a substitution rule from a dict of rules, 
    according to its weighted probality. 'rules' is expected to be a dict 
    where the substition string is the 'key' and the 'value' is the rule weight
    """
    rand = random.random()
    prob = 0
    ...
    for rule in rules.keys():     # iterate over rule choices
        ...
        if ((rand * tot) < prob): # compare total with scaled random value
            return(rule)

def produce(axiom, rules):
    """
    The single rule substitution utility, that uses type to check for dict or
    str as key value, else declares 'Unrecognized grammar'. Does not throw 
    exception!!!
    """
    str_buf = [] # list is much more efficient than string concatenation
...
def __testWeighting(rules, key, total):     
    ...
    # calculate each word frequency in generated list (NB: does not test
    # the randomness of order though)
    freqD2 = {}
