...
class POVFile(object):    
    ...
    def __init__(self, fnam = "out.pov", size = (300,  300),  *items):
        ...
        self.writeln( '// width {0} height {1}'.format(*size))
    ...
    def include(self, name):
        ...
        """
        self.writeln( '#include "{}"'.format(name))
        self.writeln()
    def declare(self, name):
        """
        Formats and writes a PovRAY include file 'name'
        """
        self.writeln( '#declare {}'.format(name))
...
class Vector(object):
    ...
    def __init__(self, *args):
        if len(args) == 1:
            ...
            self.vect = args
    def __str__(self):
        return "<{}>".format(", ".join([str(x) for x in self.vect]))
    def __repr__(self):
    ...
    def write(self, outfile):
        ...
        for key, val in list(self.kwargs.items()):
            if type(val) == tuple or type(val) == list:
                val = Vector(*val)
                outfile.writeln( "{0} {1}".format(key, val))
            else:
                outfile.writeln( "{0} {1}".format(key, val) )
        outfile.block_end()
